...
- the valid phase sample count is less than `fs * 60 - 10`, where `fs` is the
  detected sampling rate in hertz (exactly 10 dropped samples is allowed); or
- the satellite is GLONASS (its PRN begins with `R`).

`s4_quality_flag_1`, `s4_quality_flag_2`, and `s4_quality_flag_3` are set to `1`
when the valid C/N0 sample count is less than `fs * 60 * 0.8`. A minute with
exactly 80% of its expected samples remains good.
